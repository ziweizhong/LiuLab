...
for s in eSets:
	...
	with open ('0Output-%s-enrich.txt'%(str(s)),'w') as outFit:
		outFit.write("Name,Fitness,Count t0,Count t1,Count t2,Total Count,R-squared,Number of Mutations")
		...
		for muts in results:
			...
			outFit.write("%s,%f,%d,%d,%d,%d,%f,%d"%(str(muts),slope*100,counts2[0],counts2[2], counts2[1],totalCount,r_value**2,numMuts))
