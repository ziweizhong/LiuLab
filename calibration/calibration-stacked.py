...
This code should be used in conjunction with stackplot and is currently very crude.
This script will write the detector_values and measured_od to outfile_vial%d, which stackplot.py will plot together. Each time the script is run, it will append the x and y data to the respective vial file so that different calibration runs can be plotted on the same graph.
"""
...
def graph_2d_data(func, measured_data, medians, standard_deviations, coefficients, fit_name, fit_type, space_min, space_max, space_step):
    ...
    for i in range(16):
        ...
        # output to file
        fileName = "outfile_vial%d"%(i)
        outFile = open(fileName, "a+")
        for item in medians[i]:
            outFile.write('%f,'%item)
        outFile.write("\n")
        for item in measured_data[i]:
            outFile.write('%f,'%item)
        outFile.write("\n")
        outFile.close()
