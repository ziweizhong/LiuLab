...
# python calibrate.py -a <ip.address> -n <file name in quotes> -t <type:sigmoid/linear/3d/exp> -f <name.after.fit> -p <od_135 vs od_90>

ZZ 2021-10-23 updates:
Code has been updated to enable generation of linear and
exponential fits of detector values to OD. These can be called
via the -linear_od and -exp flags for fit type (after -t).
The data will similarly saved onto eVOLVER, and can be set as
active via the electron GUI interface. However, it is not
certain whether electron GUI will displace the correct conversion of raw data and OD.
Once any calibration is selected as active on the electron GUI, the eVOLVER.py script should automatically pull calibration data when run.
Alternative, the calibration file can be manually extracted from the calibrations.json file located on pi@10.0.0.3:~/evolver/evolver/calibrations.json
"""
...
VALID_FIT_TYPES = ['sigmoid', 'linear', 'constant', '3d', 'linear_od','exp']
...
# ZZ ADDED linear OD fit and exp_fit
def linear_fit_od(calibration, fit_name, params, graph = True):
    coefficients = []
    ...
    for i in range(16):
        #paramlin, cov = curve_fit(linear, measured_data[i], medians[i])
        paramlin, cov = curve_fit(linear, medians[i], measured_data[i])
        print("\n\nMeasured Data\n")
        print(measured_data[i])
        print("\n\nMedians\n")
        print(medians[i])
        print("y = ax + b; a = %f, b = %f" %(paramlin[0], paramlin[1]))

        coefficients.append(paramlin.tolist())
    print(coefficients)

    if graph:
        graph_2d_data(linear, measured_data, medians, standard_deviations, coefficients, fit_name, 'linear_od', 0.9*min([min(sublist) for sublist in medians]), max([max(sublist) for sublist in medians]), 500)
    return create_fit(coefficients, fit_name, "linear_od", time.time(), params)

def exp_fit(calibration, fit_name, params, graph = True):
    coefficients = []
    ...
    for i in range(16):
        paramsig, paramlin = curve_fit(exp, measured_data[i], medians[i], p0 = [10000, 500, 100, -5000])
        coefficients.append(np.array(paramsig).tolist())
    print(coefficients)

    if graph:
        graph_2d_data(exp, measured_data, medians, standard_deviations, coefficients, fit_name, 'exp', 0, max([max(sublist) for sublist in measured_data]), 500)
    return create_fit(coefficients, fit_name, "exp", time.time(), params)
...
def linear_fit(calibration, fit_name, params, graph = True):
    ...
    if graph:
        # IF WE WANT LINEAR OD, UNCOMMENT THE NEXT LINE
        # graph_2d_data(linear, medians, measured_data, standard_deviations, coefficients, fit_name, 'linear', 0, 70000, 50)

        # OLD GRAPH 2D DATA CALL FOR TEMPERATURE
...
def graph_2d_data(func, measured_data, medians, standard_deviations, coefficients, fit_name, fit_type, space_min, space_max, space_step):
    ...
    #print("Space min %f" %space_min)
    #print("Space max %f" %space_max)
    ...
    for i in range(16):
        ...
        ax[i // 4, (i % 4)].plot(medians[i],measured_data[i], 'o', markersize=3, color='black')
        #ax[i //4, (i % 4)].errorbar(medians[i],measured_data[i], yerr=standard_deviations[i], fmt='none')
        ax[i // 4, (i % 4)].plot(linear_space, func(linear_space, *coefficients[i]), markersize = 1.5, label = None)
        print(fit_type)
        if fit_type == 'linear_od':
            ax[i // 4, (i % 4)].set_xlim([0.95*min(medians[i]),1.05*max(medians[i])])
            ax[i // 4, (i % 4)].set_ylim([0,2])

        #print("vial %d: xlim: %f - %f" %(i,min(medians[i]),max(medians[i])))
...
def process_vial_data(calibration, param = None):
    ...
    for i, vial_data in enumerate(vial_datas):
        ...
        for vial in vial_data:
            ...
            print("vial %d"%i)
            for point in vial:
                print("Mean: %d" % np.mean(point))
                print("Median: %d" %np.median(point))
                print("Difference: %.2f"%(np.mean(point)-np.median(point)))
...
if __name__ == '__main__':
    ...
    if cal_name is not None and not get_names:
        ...
        elif fit_type == 'linear_od':
            fit = linear_fit_od(calibration, fit_name, params)
