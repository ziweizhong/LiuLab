...
def calibrate_OD(eVOLVER, input_data, vials, elapsed_time):
    ...
    if old_data.ndim < 2 or old_data.shape[0] < 16:
        ...
        print ("Data %d of 16 saved!"% old_data.shape[0])
...
def morbidostat(eVOLVER, input_data, vials, elapsed_time):
    ...
    if elapsed_time > 10: # initial time for growth, hours
        ...
        if ((elapsed_time - lastPump)*3600) > (TIME_BETWEEN_DILUTIONS * 60): # convert to seconds and compare
            ...
            for x in mstat_vials:
                ...
                if averageOD > DILUTION_OD:
                    ...
                    # NEW PUMP Command
                    # NOTE THAT THE "-8" IS FOR TESTING PURPOSES ONLY!!!!
                    # #ZZ#
                    # No drug media influx
                    MESSAGE[x-8] = str(numDils * VOLUME_PER_DILUTION * (1-PIDControl))
                    # Drug media influx
                    MESSAGE[x+32-8] = str(numDils * VOLUME_PER_DILUTION * (PIDControl))
                    # Efflux pump
                    MESSAGE[x+16-8] = str(numDils * VOLUME_PER_DILUTION + 5)
                    """
